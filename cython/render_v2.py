...
from renderer_v2 import render_pixels
import time 
...
def render_attractors(xl, yl, zl, coeff, dimension, seed, tag, alpha = 0.0075, xres = 3200, yres = 1800):
	...
	if not np.isnan(xrng):
		...
		for k in range(3):
			render[:,:,k] = np.interp(render[:,:,k], (render[:,:,k].min(), render[:,:,k].max()), (0, 1))

		print(f'Interpolated color range {time.time()-start:.1f} seconds')
